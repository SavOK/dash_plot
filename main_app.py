...
# data for plot
heatmap_fig = read_heatmap_figure()
...
def get_data_histogram_details(
...
def plot_histogram_details(data: pd.DataFrame, domain_id: AnyStr) -> go.Figure:
    """ 
    Plot histogram of details data
    Arguments:
        data {pd.DataFrame} -- data to be printed
        domain_id {AnyStr} -- domain id used in header part
    Returns:
        go.Figure -- figure
    """
    color_discrete_map = {
...
def get_group_data(X: AnyStr, Y: AnyStr, main_df: pd.DataFrame = main_df) -> Tuple:
    """
    get data for xgroup 1, group 2 and 1 vs. 2
    Arguments:
        X {AnyStr} -- X group id 1
        Y {AnyStr} -- X group id 2
    Keyword Arguments:
        main_df {pd.DataFrame} -- main table (default: {main_df})
    Returns:
        Tuple -- (df1, df2, df3) if X gr== Y gr df2, and df3 is None 
        if cannot find data for any of x-group return None
    """
    X = int(X)
    Y = int(Y)
    if X == Y:
...
def get_data_plot_group(X: AnyStr, Y: AnyStr) -> pd.DataFrame:
    """summaries data in one dataframe 
    Arguments:
        X {AnyStr} -- group id 1
        Y {AnyStr} -- group id 2
    Returns:
        pd.DataFrame -- plot data or None if no info found for x group
    """
...
def plot_histogram_group(df_plot: pd.DataFrame, info_dict: dict) -> go.Figure:
    """ plots figure histogram fo group
    Arguments:
        df_plot {pd.DataFrame} -- plot dataframe
        info_dict {dict} -- dictionary with description info
    Returns:
        go.Figure -- Figure or None
    """
...
def details_dev_output(clickData, old_output):
    ...
    if info_dict is None:
        raise PreventUpdate
    plot_1_df, plot_2_df = get_data_histogram_details(info_dict)
...
def group_dev_output(clickData, old_output):
    ...
    if info_dict is None:
        raise PreventUpdate
    df_plot = get_data_plot_group(info_dict["X"], info_dict["Y"])
    if df_plot is None:
        raise PreventUpdate
